...
namespace RAM.Import
{
    public class RAMImporter
    {
        ...
        {
            var importer = new MetadataImporter(_model);
            model.Metadata = importer.Import();
        }

        private void ImportFloorTypes(BaseModel model)
        {
            var importer = new FloorTypeImporter(_model);
        ...
        {
            var importer = new MaterialImporter(_model);
        ...
        private void ImportProperties(BaseModel model)
        {
            ...
            var slabImporter = new SlabPropertiesImporter(_model, _materialIdMap);
            ...
            var frameSectionImporter = new FrameSectionPropertiesImporter(_model, _materialIdMap);
        ...
        private void ImportElements(BaseModel model)
        {
            ...
            // Import floors
            var floorImporter = new FloorImporter(_model, _floorTypeIdMap, _floorPropertyIdMap);
        ...
        private void ImportLoads(BaseModel model)
        {
            ...
            var loadDefinitionImporter = new LoadDefinitionImporter(_model);
            ...
            var loadCombinationImporter = new LoadCombinationImporter(_model, _loadCaseIdMap);
            model.Loads.LoadCombinations = loadCombinationImporter.Import();

            // Import surface loads
            var surfaceLoadImporter = new SurfaceLoadImporter(_model, _loadCaseIdMap);
