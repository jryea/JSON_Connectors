...
namespace Revit.Import.Properties
{
    public class FloorPropertiesImport
    {
        ...
        public DB.FloorType FindOrCreateFloorType(FloorProperties floorProps, BaseModel model)
        {
            ...
            Debug.WriteLine($"Finding floor type for: {floorProps.Type}, Thickness: {floorProps.Thickness}");

            switch (floorProps.Type)
            {
                case StructuralFloorType.FilledDeck:
                    selectedType = FindOrCreateFilledDeckType(floorProps);
                    break;
                case StructuralFloorType.UnfilledDeck:
                    selectedType = FindOrCreateUnfilledDeckType(floorProps);
                    break;
                case StructuralFloorType.Slab:
                    selectedType = FindOrCreateSlabType(floorProps);
                    break;
                default:
        ...
        private DB.FloorType FindOrCreateFilledDeckType(FloorProperties floorProps)
        {
            // floorProps.Thickness is in inches, convert to feet for Revit API
            double totalThicknessInches = floorProps.Thickness;
            double deckThicknessInches = floorProps.DeckProperties?.RibDepth ?? 3.0;
            double concreteThicknessInches = totalThicknessInches - deckThicknessInches;

            string concreteThicknessStr = FormatThickness(concreteThicknessInches);
            string deckThicknessStr = FormatThickness(deckThicknessInches);
            ...
            if (_defaultDeckType != null)
            {
                ...
                return CreateFloorTypeWithThickness(_defaultDeckType, floorProps.Thickness, newTypeName);
            }
            ...
            if (deckFloorType != null)
            {
                ...
                return CreateFilledDeckFloorType(deckFloorType, floorProps, newTypeName);
            }

            Debug.WriteLine("No suitable deck type found, falling back to concrete slab");
            return FindOrCreateSlabType(floorProps);
        }

        private DB.FloorType FindOrCreateUnfilledDeckType(FloorProperties floorProps)
        {
            // floorProps.Thickness is in inches, use directly for naming
            double deckThicknessInches = floorProps.DeckProperties?.RibDepth ?? 3.0;
            string deckThicknessStr = FormatThickness(deckThicknessInches);
            ...
            DB.FloorType newDeckType = CreateUnfilledDeckFloorType(floorProps, nameVariations[0]);
            if (newDeckType != null)
            {
                return newDeckType;
            }
        ...
        private DB.FloorType CreateUnfilledDeckFloorType(FloorProperties floorProps, string newTypeName)
        {
            try
            {
                ...
                var deckProfile = FindDeckProfile(floorProps);
                if (deckProfile == null)
                {
                ...
                if (ModifyUnfilledDeckStructure(newFloorType, floorProps, deckProfile))
                {
        ...
        private DB.ElementType FindDeckProfile(FloorProperties floorProps)
        {
            try
            {
                ...
                {
                    var matchingType = FindBestDeckTypeByHeight(imegDeckIndents.Family, floorProps, false);
                ...
                if (imegDeck != null)
                {
                    var matchingType = FindBestDeckTypeByHeight(imegDeck.Family, floorProps, false);
                    if (matchingType != null)
                    {
                ...
                {
                    var matchingType = FindBestDeckTypeByHeight(deckProfile.Family, floorProps, false);
                    if (matchingType != null)
                    {
        ...
        private DB.ElementType FindBestDeckTypeByHeight(DB.Family family, FloorProperties floorProps, bool isComposite)
        {
            try
            {
                ...
                // Find best match by height parameter
                double targetHeightInches = floorProps.DeckProperties?.RibDepth ?? 3.0;
                ...
                foreach (var type in filteredTypes)
                {
                    ...
                    if (heightParam != null && heightParam.HasValue)
                    {
                        ...
                        double difference = Math.Abs(paramValueInches - targetHeightInches);

                        Debug.WriteLine($"Type '{type.Name}': height = {paramValueInches:F2}\" (target: {targetHeightInches:F2}\"), diff = {difference:F2}");
        ...
        private bool ModifyUnfilledDeckStructure(DB.FloorType floorType, FloorProperties floorProps, DB.ElementType deckProfile)
        {
            try
            {
                ...
                // Set the deck thickness and profile
                if (cs.LayerCount > 0)
                {
                    double deckThicknessInches = floorProps.DeckProperties?.RibDepth ?? 3.0;
                    double deckThicknessFeet = deckThicknessInches / 12.0;

                    cs.SetLayerWidth(0, deckThicknessFeet);
        ...
        private DB.FloorType FindOrCreateSlabType(FloorProperties floorProps)
        {
            // floorProps.Thickness is in inches, use directly for naming
            double thicknessInches = floorProps.Thickness;
            string thicknessStr = FormatThickness(thicknessInches);
            ...
            if (_defaultConcreteType != null)
            {
                ...
                return CreateFloorTypeWithThickness(_defaultConcreteType, floorProps.Thickness / 12.0, newTypeName);
            }
        ...
        private DB.FloorType CreateFilledDeckFloorType(DB.FloorType baseFloorType, FloorProperties floorProps, string newTypeName)
        {
            try
            {
                ...
                // Modify the compound structure for filled deck
                DB.CompoundStructure cs = newFloorType.GetCompoundStructure();
                if (cs != null && cs.HasStructuralDeck)
                {
                    double targetTotalThickness = floorProps.Thickness / 12.0; // Convert inches to feet
                    double deckThicknessInches = floorProps.DeckProperties?.RibDepth ?? 0;
                    double deckThickness = deckThicknessInches / 12.0; // Convert to feet
                    ...
                    if (deckLayerIndex >= 0)
                    {
                        ...
                        cs.SetLayerWidth(deckLayerIndex, Math.Max(deckThickness, 0.01));
                    }
